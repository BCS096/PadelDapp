...
const App = ({ addressClub }) => {
    ...
    useEffect(() => {
        if (padelTokenService) {
            padelTokenService.getAllowSell(addressClub).then((res) => {
                setNumPDT(res);
            });
        }
    }, [padelTokenService, addressClub]);
    ...
    const handlePDTVenta = async () => {
        ...
        try {
            await padelTokenService.transfer(account, addressClub, inputValue);
            const newNumPDT = await padelTokenService.getAllowSell(addressClub);
            setNumPDT(newNumPDT);
            setModal({
                visible: true,
